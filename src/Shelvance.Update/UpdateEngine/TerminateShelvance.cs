...
{
    public interface ITerminateShelvance
    {
        void Terminate(int processId);
    }

    public class TerminateShelvance : ITerminateShelvance
    {
        private readonly IServiceProvider _serviceProvider;
        private readonly IProcessProvider _processProvider;
        private readonly Logger _logger;

        public TerminateShelvance(IServiceProvider serviceProvider, IProcessProvider processProvider, Logger logger)
        {
        ...
        public void Terminate(int processId)
        {
            if (OsInfo.IsWindows)
            {
                    ...
                    && _serviceProvider.IsServiceRunning(ServiceProvider.SERVICE_NAME))
                {
                    try
                    {
                        _logger.Info("Shelvance Service is installed and running");
