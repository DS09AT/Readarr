...
namespace NzbDrone.Core.Download.Clients.Qnap
{
    public class HttpQnap : HttpClientBase<QnapSettings>
    {
        ...
        public HttpQnap(IQnapProxy proxy,
        ...
        protected override string AddFromHttpUrl(RemoteBook remoteBook, string url)
        {
            _logger.Info("Adding HTTP download [{0}] to QNAP queue.", remoteBook.Release.Title);
            ...
            var hash = _proxy.AddUrl(Settings, auth.Sid, auth.Token, url, tempFolder, destinationFolder);

            _logger.Debug(
                "QNAP Download Station added HTTP download: {0} with hash {1} (temp: {2}, move: {3})",
                remoteBook.Release.Title,
                hash,
                string.IsNullOrWhiteSpace(tempFolder) ? "default" : tempFolder,
                string.IsNullOrWhiteSpace(destinationFolder) ? "default" : destinationFolder);
        ...
        public override IEnumerable<DownloadClientItem> GetItems()
        {
            ...
            {
                // Filter: Only HTTP/FTP tasks (State < 100)
                if (task.State >= 100)
                {
                    continue;
                }
        ...
        private DownloadItemStatus MapStatus(int stateCode)
        {
            return stateCode switch
            {
                ...
                3 => DownloadItemStatus.Paused,
                4 => DownloadItemStatus.Completed,
                5 => DownloadItemStatus.Failed,
