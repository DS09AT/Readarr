...
namespace Shelvance.Host
{
    ...
    public class SingleInstancePolicy : ISingleInstancePolicy
    {
        ...
        public void PreventStartIfAlreadyRunning()
        {
            if (IsAlreadyRunning())
            {
                _logger.Warn("Another instance of Shelvance is already running.");
        ...
        {
            foreach (var processId in GetOtherShelvanceProcessIds())
            {
        ...
        public void WarnIfAlreadyRunning()
        {
            if (IsAlreadyRunning())
            {
                _logger.Debug("Another instance of Shelvance is already running.");
            }
        }

        private bool IsAlreadyRunning()
        {
            return GetOtherShelvanceProcessIds().Any();
        }

        private List<int> GetOtherShelvanceProcessIds()
        {
            try
            {
                var currentId = _processProvider.GetCurrentProcess().Id;

                var otherProcesses = _processProvider.FindProcessByName(ProcessProvider.SHELVANCE_CONSOLE_PROCESS_NAME)
                                                     .Union(_processProvider.FindProcessByName(ProcessProvider.SHELVANCE_PROCESS_NAME))
                                                     .Select(c => c.Id)
                ...
                {
                    _logger.Info("{0} instance(s) of Shelvance are running", otherProcesses.Count);
                }
            ...
            {
                _logger.Warn(ex, "Failed to check for multiple instances of Shelvance.");
                return new List<int>();
            }
        }
    }
