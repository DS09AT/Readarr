...
namespace Shelvance.Core.Backup
{
    ...
    public class BackupService : IBackupService, IExecute<BackupCommand>
    {
        ...
        public static readonly Regex BackupFileRegex = new Regex(@"shelvance_backup_(v[0-9.]+_)?[._0-9]+\.zip", RegexOptions.Compiled | RegexOptions.IgnoreCase);
                             ...
                             Logger logger)
        {
            ...
            _backupTempFolder = Path.Combine(_appFolderInfo.TempFolder, "shelvance_backup");
        }
        ...
        public void Backup(BackupType backupType)
        {
            ...
            var backupFilename = $"shelvance_backup_v{BuildInfo.Version}_{dateNow:yyyy.MM.dd_HH.mm.ss}.zip";
            ...
            _diskProvider.DeleteFile(Path.Combine(_backupTempFolder, "shelvance.db-journal"));
        ...
        public void Restore(string backupFileName)
        {
            if (backupFileName.EndsWith(".zip"))
            {
                ...
                var temporaryPath = Path.Combine(_appFolderInfo.TempFolder, "shelvance_backup_restore");
                ...
                foreach (var file in _diskProvider.GetFiles(temporaryPath, false))
                {
                    ...
                    if (fileName.Equals("shelvance.db", StringComparison.InvariantCultureIgnoreCase))
                    {
