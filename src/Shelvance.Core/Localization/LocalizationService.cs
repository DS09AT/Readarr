...
namespace Shelvance.Core.Localization
{
    ...
    public class LocalizationService : ILocalizationService, IHandleAsync<ConfigSavedEvent>
    {
        ...
        {
            tokens.TryAdd("appName", "Shelvance");
        ...
        private async Task<Dictionary<string, string>> GetDictionary(string prefix, string culture, string baseFilename)
        {
            ...
            await CopyInto(dictionary, baseFilenamePath, logAsError: true).ConfigureAwait(false);

            if (culture.Contains("_"))
            {
                var languageBaseFilenamePath = Path.Combine(prefix, GetResourceFilename(culture.Split('_')[0]));
                await CopyInto(dictionary, languageBaseFilenamePath, logAsError: false).ConfigureAwait(false);
            }

            await CopyInto(dictionary, alternativeFilenamePath, logAsError: false).ConfigureAwait(false);

            return dictionary;
        }

        private async Task CopyInto(IDictionary<string, string> dictionary, string resourcePath, bool logAsError = false)
        {
            if (!File.Exists(resourcePath))
            {
                ...
                {
                    _logger.Debug("Optional translation/culture resource not found: {0}", resourcePath);
                }
