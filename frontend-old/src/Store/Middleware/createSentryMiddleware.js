...
function stripUrlBase(frame) {
  if (frame.filename && window.Radarr.urlBase) {
    frame.filename = frame.filename.replace(window.Shelvance.urlBase, '');
  }
  return frame;
}
...
export default function createSentryMiddleware() {
  ...
  const dsn = isProduction ? 'https://00000000000000000000000000000000@sentry.example.com/7' :
    'https://00000000000000000000000000000000@sentry.example.com/8';
